...
function dashboard() {
  ...
  return shell(`<main>
    <section class="dashboard-hero"><div class="dashboard-copy"><p class="eyebrow"><span>13 ${state.lang === 'zh' ? '個主題' : 'TOPICS'}</span> · ${state.lang === 'zh' ? '由基礎到安全上線' : 'FOUNDATIONS TO SAFE RELEASE'}</p><h1>${state.lang === 'zh' ? '唔只識用 AI。<br>真正理解佢點運作。' : 'Don’t just use AI.<br>Understand how it works.'}</h1><p class="hero-lede">${state.lang === 'zh' ? '用可控制動畫、參數實驗、真實失敗案例同精準回饋，由一個安全 request 行到完整 AI system。' : 'Learn through controlled visuals, parameter labs, real failure cases, and precise feedback—from one safe request to a complete AI system.'}</p><div class="hero-actions"><a class="primary-action" href="#/lesson/${current.slug}">${mastered ? ui.continue : ui.start}<span>→</span></a><a class="secondary-action" href="#/map">${ui.map}</a></div><div class="trust-row"><span>✓ ${state.lang === 'zh' ? '本機儲存進度' : 'Local progress'}</span><span>✓ ${state.lang === 'zh' ? '無需安裝、無付費 API' : 'No install, no paid API'}</span><span>✓ Node 18.18</span></div></div>
...
function learnTab(topic) {
  return `<div class="lesson-content"><section class="orientation-card"><p class="eyebrow">${state.lang === 'zh' ? '開場問題' : 'OPENING PROBLEM'}</p><h2>${text(topic.orientation.openingProblem)}</h2><div class="one-sentence"><b>${state.lang === 'zh' ? '一句講清' : 'ONE-SENTENCE MODEL'}</b><p>${text(topic.orientation.oneSentence)}</p></div><div class="objectives"><h3>${state.lang === 'zh' ? '完成後你可以' : 'YOU WILL BE ABLE TO'}</h3>${topic.orientation.objectives.map((objective,index) => `<div><span>${index+1}</span><p><b>${text(objective.statement)}</b><small>${text(objective.evidence)}</small></p></div>`).join('')}</div></section>
    ...
    <section class="deep-dive"><p class="eyebrow">DEEP DIVE</p><h2>${state.lang === 'zh' ? '由概念落到機制' : 'From concept to mechanism'}</h2><p>${text(topic.explanation.deepDive)}</p><ol>${topic.explanation.mechanisms.map((item) => `<li>${text(item)}</li>`).join('')}</ol></section>${visualLab(topic)}

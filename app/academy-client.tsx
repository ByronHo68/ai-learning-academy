...
import { Check, ChevronDown, Code2, Copy, Lightbulb, Sparkles, TestTube2 } from 'lucide-react';
...
function KineticBackdrop() {
  ...
  return <div className="kinetic-backdrop" aria-hidden="true">
    {[0,1,2].map((index)=><motion.i key={index} animate={reduced?undefined:{rotate:360,scale:[1,1.08,1]}} transition={{duration:18+index*5,repeat:Infinity,ease:'linear'}}/>)
    }
    {[0,1,2,3,4].map((index)=><motion.b key={index} animate={reduced?undefined:{y:[0,-18-index*3,0],opacity:[.18,.65,.18]}} transition={{duration:3.4+index*.45,repeat:Infinity,ease:'easeInOut',delay:index*.28}}/>)
    }
  </div>;
}
...
function DashboardIntelligence({ state }: { state:AcademyState }) {
  ...
  return <section className="dashboard-intelligence" aria-labelledby="content-health-title">
    <div className="dashboard-status-grid">
      ...
      <article><span>03</span><p>{lang==='zh'?'跨主題能力':'Cross-topic readiness'}</p><strong>{state.cumulative.bestScore}%</strong><small>{state.cumulative.capstoneComplete?(lang==='zh'?'設計檢查已完成':'Design checkpoint complete'):(lang==='zh'?'設計檢查未完成':'Design checkpoint pending')}</small><Link href="/assessment">{lang==='zh'?'開始總評':'Start assessment'} →</Link></article>
    </div>
...
function Dashboard({ state, setState }: { state:AcademyState; setState:React.Dispatch<React.SetStateAction<AcademyState>> }) {
  ...
  return (
    <Shell state={state} setState={setState}>
      <main>
        <motion.section className="dashboard-hero rich-hero" initial="hidden" animate="visible" variants={reveal} transition={{duration:.65,ease:[.2,.8,.2,1]}}>
          <KineticBackdrop/>
          <motion.div className="dashboard-copy" initial={{opacity:0,x:-28}} animate={{opacity:1,x:0}} transition={{duration:.7,delay:.08}}>
            ...
            <div className="trust-row"><span>✓ 65 {lang === 'zh' ? '個概念工作坊' : 'concept workshops'}</span><span>✓ {lang === 'zh' ? '互動 code 同動畫' : 'Interactive code & motion'}</span><span>✓ {lang === 'zh' ? '鍵盤與減少動態支援' : 'Keyboard & reduced motion'}</span></div>
          </motion.div>
          <motion.div className="progress-orbit" aria-label={`${text.progress}: ${percent}%`} initial={{opacity:0,scale:.92,rotate:1.5}} animate={{opacity:1,scale:1,rotate:0}} transition={{duration:.75,delay:.18,type:'spring',bounce:.25}} whileHover={{y:-6,rotate:-.35}}>
            ...
            <div className="mini-path">
              {topics.slice(0,6).map((topic,index) => <motion.div className={`mini-node ${state.topicProgress[topic.id].status}`} key={topic.id} initial={{opacity:0,x:18}} animate={{opacity:1,x:0}} transition={{delay:.28+index*.07}}><b>{String(index+1).padStart(2,'0')}</b><span>{pick(topic.title,lang)}</span><em>{stateLabel(state.topicProgress[topic.id].status,lang)}</em></motion.div>)}
            </div>
        ...
        <section className="course-section" id="path">
          ...
          <div className="full-module-grid">{topics.map((topic,index) => {
            const progress=state.topicProgress[topic.id];
            return <motion.article className={`course-card ${progress.status}`} key={topic.id} initial={{opacity:0,y:26}} whileInView={{opacity:1,y:0}} viewport={{once:true,amount:.18}} transition={{duration:.45,delay:(index%3)*.07}} whileHover={{y:-8,rotate:index%2?.35:-.35}}>
...
function VisualLab({ topic, lang }: { topic:Topic; lang:Lang }) {
  ...
  return <section className={`visual-lab ${topic.visual.kind}`} id={`${topic.id}-visual`} aria-labelledby={`${topic.id}-visual-title`}>
    ...
    <div className="visual-stage" role="img" aria-label={pick(topic.visual.textAlternative,lang)}>
      {topic.visual.steps.map((item,index)=><motion.div className={`visual-node ${index===step?'active':''} ${index<step?'complete':''}`} key={item.id} animate={reducedMotion?undefined:{y:index===step?-6:0,scale:index===step?1.035:1}} transition={{type:'spring',stiffness:320,damping:22}}><span>{String(index+1).padStart(2,'0')}</span><b>{pick(item.label,lang)}</b>{index<topic.visual.steps.length-1&&<i aria-hidden="true">→</i>}</motion.div>)}
    </div>
    ...
    <AnimatePresence mode="wait" initial={false}><motion.div className="visual-narration" key={`${step}-${parameter}`} initial={reducedMotion?false:{opacity:0,x:18}} animate={{opacity:1,x:0}} exit={reducedMotion?undefined:{opacity:0,x:-18}} transition={{duration:.26}}><span>{String(step+1).padStart(2,'0')}</span><p>{pick(active.narration,lang)} {topic.visual.parameter ? `${pick(topic.visual.parameter.label,lang)}: ${parameter}${topic.visual.parameter.unit}.` : ''}</p></motion.div></AnimatePresence>
    <div className="visual-readout" aria-label={lang==='zh'?'參數結果':'Parameter results'}>{metrics.map((item)=><motion.div key={item.label} className={item.direction} animate={reducedMotion?undefined:{y:[0,-2,0]}} transition={{duration:.34}}><p><span>{item.label}</span><b>{item.display}</b></p><div><motion.i initial={false} animate={{width:`${item.value}%`}} transition={{duration:.38,ease:'easeOut'}}/></div></motion.div>)}</div>
...
const prismLanguage=(language:string):Language=>language.toLowerCase()==='python'?'python':'javascript';

function RichCodeExample({ detail, lang, index }:{ detail:ConceptDetail; lang:Lang; index:number }) {
  ...
  return <div className="rich-code-example">
    <header><div><Code2 size={15}/><span><small>{detail.language}</small><b>{pick(detail.codeTitle,lang)}</b></span></div><button onClick={copyCode} type="button" aria-label={lang==='zh'?'複製 code':'Copy code'}>{copied?<Check size={14}/>:<Copy size={14}/>} {copied?(lang==='zh'?'已複製':'Copied'):(lang==='zh'?'複製':'Copy')}</button></header>
    <Highlight theme={themes.nightOwl} code={detail.code} language={prismLanguage(detail.language)}>
      {({className,style,tokens,getLineProps,getTokenProps})=><pre className={className} style={style}>{tokens.map((line,lineIndex)=><div {...getLineProps({line})} key={lineIndex}><span className="code-line-number">{String(lineIndex+1).padStart(2,'0')}</span>{line.map((token,tokenIndex)=><span {...getTokenProps({token})} key={tokenIndex}/>)}</div>)}</pre>}
    </Highlight>
    <footer><span>{String(index+1).padStart(2,'0')}</span><p>{lang==='zh'?'教學用最小例子：部署前加入真實 schema、授權、錯誤處理同測試。':'Minimal teaching example: add real schemas, authorization, error handling, and tests before deployment.'}</p></footer>
  </div>;
}
...
function ConceptWorkshop({ topic,lang }:{ topic:Topic;lang:Lang }) {
  ...
  return <section className="rich-concept-workshop" aria-labelledby={`${topic.id}-concept-workshop`}>
    ...
    <div className="rich-concept-list">{details.map((detail,index)=>{
      ...
      return <motion.article layout className={open?'open':''} key={detail.title.en} initial={reduced?false:{opacity:0,x:index%2?-24:24}} whileInView={{opacity:1,x:0}} viewport={{once:true,amount:.18}} transition={{duration:.48,delay:index*.08}}>
        <button className="rich-concept-summary" type="button" onClick={()=>setExpanded(open?null:index)} aria-expanded={open}>
    ...
    <main className="lesson-layout">
      ...
      <div className="lesson-main">
        ...
        {tab==='learn'&&<div className="lesson-content">
          ...
          <ConceptWorkshop topic={topic} lang={lang}/>
        ...
        {tab==='recap'&&<section className="recap-page">
          ...
          {nextTopic?<Link className="wide-next" href={`/learn/${nextTopic.slug}`}>{lang==='zh'?'下一主題':'Next topic'}: {pick(nextTopic.title,lang)} <span>→</span></Link>:<Link className="wide-next" href="/assessment">{lang==='zh'?'完成總評、設計檢查同實作 brief':'Complete the assessment, design checkpoint, and build brief'} <span>→</span></Link>}
        </section>}
...
function AssessmentPage({ state,setState }: { state:AcademyState; setState:React.Dispatch<React.SetStateAction<AcademyState>> }) {
  ...
  const productionBuild=[
    {zh:'API 合約：登入、輸入 schema、5 秒 timeout、取消、明確錯誤；key 只在 backend。',en:'API contract: sign-in, input schema, five-second timeout, cancellation, explicit errors; key only on the backend.'},
    {zh:'資料生命週期：合成訂單、最新政策、tenant filter、更新/刪除及快取失效。',en:'Data lifecycle: synthetic orders, current policy, tenant filtering, updates/deletion, and cache invalidation.'},
    {zh:'Agent 控制：只可讀自己訂單、草擬退款；真正寫入需人批核及 idempotency key。',en:'Agent controls: read only the user’s order and draft a refund; writes require human approval and an idempotency key.'},
    {zh:'評測：有版本嘅正常、粵英雙語、資料缺失、越權同 injection cases；critical failure 令 CI fail。',en:'Evaluation: versioned normal, bilingual, missing-data, unauthorized, and injection cases; critical failures fail CI.'},
    {zh:'維運：traceId、model/prompt 版本、p95、錯誤率、token/成本；模擬 429、timeout 同壞 JSON。',en:'Operations: trace ID, model/prompt versions, p95, error rate, tokens/cost; inject 429s, timeouts, and malformed JSON.'},
    {zh:'發佈：canary stop gate、上一個穩定版本、rollback runbook，同脫敏 incident regression。',en:'Release: canary stop gate, previous stable version, rollback runbook, and redacted incident regression.'},
  ];
  ...
  return <Shell state={state} setState={setState}><main className="utility-page assessment-shell">
    <div className="utility-hero"><p className="eyebrow">CUMULATIVE ASSESSMENT + PRODUCTION BUILD</p><h1>{lang==='zh'?'將 13 個主題連成一個決定':'Turn 13 topics into one sound decision'}</h1><p>{lang==='zh'?'先做 13 題跨主題情境題，再完成設計檢查同實作 brief。選擇題只證明理解，唔等於已部署 production 系統。':'Complete 13 cross-topic scenarios, then the design checkpoint and hands-on build brief. Multiple-choice answers demonstrate understanding, not a verified production deployment.'}</p></div>
    <div className="assessment-overview"><div><span>{lang==='zh'?'最佳總評':'Best assessment'}</span><strong>{state.cumulative.bestScore}%</strong></div><div><span>{lang==='zh'?'總評次數':'Attempts'}</span><strong>{state.cumulative.attempts.length}</strong></div><div><span>{lang==='zh'?'設計檢查':'Design checkpoint'}</span><strong>{state.cumulative.capstoneComplete?'✓':'○'}</strong></div></div>
    ...
    <section className="capstone-lab" aria-labelledby="capstone-title"><header><p className="eyebrow">PART 2 · DESIGN CHECKPOINT</p><h2 id="capstone-title">{lang==='zh'?'上線一個安全退款助手':'Release a safe refund assistant'}</h2><p>{lang==='zh'?'情境：客服助手要讀最新政策、查登入客戶訂單、提出退款，並喺高風險動作前停低。':'Scenario: a support assistant reads current policy, checks the signed-in user’s order, proposes a refund, and pauses before high-risk action.'}</p></header><div>{capstone.map((item,index)=><fieldset key={item.id}><legend><span>{index+1}</span>{item.prompt}</legend>{item.options.map((option,optionIndex)=><label key={option}><input type="radio" name={`capstone-${item.id}`} checked={capstoneAnswers[item.id]===optionIndex} onChange={()=>{setCapstoneAnswers((value)=>({...value,[item.id]:optionIndex}));setCapstoneSubmitted(false)}}/><span className="radio-ui"/><b>{option}</b></label>)}{capstoneSubmitted&&<p className={capstoneAnswers[item.id]===item.correct?'correct':'wrong'}>{capstoneAnswers[item.id]===item.correct?'✓':'!'} {item.feedback}</p>}</fieldset>)}</div><button className="primary-button" disabled={capstone.some((item)=>capstoneAnswers[item.id]===undefined)} onClick={submitCapstone}>{lang==='zh'?'提交設計檢查':'Submit design checkpoint'}</button>{capstoneSubmitted&&<div className={`capstone-result ${capstoneComplete?'complete':'review'}`} role="status"><span>{capstoneComplete?'✓':'!'}</span><p><b>{capstoneComplete?(lang==='zh'?'設計檢查完成':'Design checkpoint complete'):(lang==='zh'?'有決定需要修正':'Some decisions need revision')}</b>{capstoneComplete?(lang==='zh'?'你已將資料新鮮度、權限、冪等性、評估同 rollback 串成一條設計路線；下一步要親手實作並驗證。':'You connected freshness, authorization, idempotency, evaluation, and rollback into a design. The next step is to build and verify it.'):(lang==='zh'?'先睇每步回饋，再重新提交；冇扣分。':'Review each explanation and resubmit without penalty.')}</p></div>}</section>
    <section className="production-build" aria-labelledby="production-build-title"><p className="eyebrow">PART 3 · HANDS-ON PRODUCTION BUILD</p><h2 id="production-build-title">{lang==='zh'?'由答啱到真正交付':'From correct answers to a working service'}</h2><p>{lang==='zh'?'比喻：設計檢查似駕駛筆試；真正上路前，仲要喺受控場地開車、處理故障同證明會安全停車。只用合成資料，唔需要真實 API key。':'Metaphor: the design checkpoint is the driving theory exam. Before driving independently, build in a controlled environment, handle failures, and prove you can stop safely. Use synthetic data; no real API key is required.'}</p><ol>{productionBuild.map((item,index)=><li key={index}><span>{String(index+1).padStart(2,'0')}</span><p>{item[lang]}</p></li>)}</ol><div className="production-build-gate"><b>{lang==='zh'?'交付證據與通過條件':'Evidence and pass criteria'}</b><p>{lang==='zh'?'提交可執行 repo、測試同 CI 結果、評測資料/版本/每個 slice 成績、脫敏 trace、威脅模型、成本與 p95 報告、rollback runbook。跨 tenant 讀取、未批核退款或 PII 洩漏必須 0 次；任何 critical failure 都不能靠平均分抵銷。呢個網站唔會自動驗證你個 repo，必須由真人 code review。':'Submit a runnable repo, tests and CI result, evaluation dataset/version/per-slice scores, redacted traces, threat model, cost and p95 report, and rollback runbook. Cross-tenant reads, unapproved refunds, and PII leaks must be zero; an average score cannot offset a critical failure. This website cannot verify your repo automatically—require human code review.'}</p></div></section>
  </main></Shell>;
}

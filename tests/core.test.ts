...
test('routes, remediation, animation accessibility, and local-only secrets contract exist',async()=>{
  ...
  for(const token of ['prefers-reduced-motion','static-alternative','aria-live="polite"','question.remediationSectionId','Play','Pause','Replay','speed']) assert.ok(client.includes(token),`missing ${token}`);
